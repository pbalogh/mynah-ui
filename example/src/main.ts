...
import {
    ...
    sampleMarkdownList,
    exampleCodeDiff,
    sampleMarkdownListWithNoCancel,
...
export const createMynahUI = (initialData?: MynahUIDataModel): MynahUI => {
    ...
    const onChatPrompt = (tabId: string, prompt: ChatPrompt) => {
        console.log('onChatPrompt got prompt', prompt);
        if (prompt.command !== undefined && prompt.command.trim() !== '') {
            switch (prompt.command) {
                ...
                case Commands.CARD_WITH_MARKDOWN_LIST:
                    ...
                    break;
                case Commands.CARD_WITH_MARKDOWN_LIST_AND_NO_CANCEL:
                    getGenerativeAIAnswer(tabId, sampleMarkdownListWithNoCancel);
                    break;
                ...
                case Commands.SHOW_STICKY_CARD:
                    mynahUI.updateStore(tabId, {
                        promptInputStickyCard: {
                            ...
                            body: undefined, //`Our [Terms and Conditions](#) are updated. Please review and read it. To accept please hit the **Acknowledge** button.`,
                            buttons: [
                                {
                                    text: 'Download chat history and the latest plan version',
                                    id: 'acknowledge',
                                    status: 'info',
                                },
                            ],
    ...
    const getGenerativeAIAnswer = (tabId: string, optionalParts?: Partial<ChatItem>[]): void => {
        ...
        if (optionalParts !== undefined) {
            const firstPart = optionalParts[0];
            showCancelButton = firstPart.showCancelButton ?? true;
        }
        mynahUI.updateStore(tabId, {
            loadingChat: true,
            cancelButtonWhenLoading: showCancelButton,
